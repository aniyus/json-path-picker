(function ($) {
   ...
   * @return string
   */
  function json2html(json, options,array_parent) {
    var html = '';
    var pickPath=""
    if(array_parent){
       pickPath="pick-path" 
    ...
    if (typeof json === 'string') {
      ...
      if (isUrl(json))
        html += '<span  class="json-string '+pickPath+'">' + json + '</a>';
      else
        html += '<span class="json-string '+pickPath+'">"' + json + '"</span>';
    }
    else if (typeof json === 'number') {
      html += '<span class="json-literal '+pickPath+'">' + json + '</span>';
    }
    else if (typeof json === 'boolean') {
      html += '<span class="json-literal '+pickPath+'">' + json + '</span>';
    }
    else if (json === null) {
      html += '<span class="json-literal '+pickPath+'">null</span>';
    }
      ...
      if (json.length > 0) {
        ...
        for (var i = 0; i < json.length; ++i) {
          ...
          html += json2html(json[i], options,true);
